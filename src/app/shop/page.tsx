...
export default function ShopPage() {
  ...
  return (
    <div className="pt-28 pb-24 bg-white min-h-screen">
      {/* Header */}
      <div className="bg-bg-muted py-12 border-b border-border mb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-xs uppercase tracking-widest text-text-secondary font-bold">
            Full Catalog
          </span>
          <h1 className="font-display font-extrabold text-4xl sm:text-6xl uppercase tracking-tighter text-text-primary mt-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Controls Bar */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pb-8 border-b border-border mb-8">
          {/* Category Tabs */}
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-none">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-pill text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
            <div className="relative flex-1 md:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-pill border border-border bg-bg-muted text-xs text-text-primary focus:outline-none focus:border-zinc-950"
              />
            </div>
            ...
            <div className="flex items-center gap-1.5 text-xs text-text-secondary">
              ...
              <select
                ...
                className="bg-bg-muted border border-border text-text-primary rounded-pill px-3 py-2 focus:outline-none cursor-pointer font-medium"
              >
              ...
              >
                <div>
                  <div className="relative w-full h-64 overflow-hidden bg-bg-muted border-b border-border">
                    ...
                    <div className="absolute top-3 right-3 px-2.5 py-1 bg-white/90 backdrop-blur-xs rounded-pill border border-border shadow-xs z-10">
                    ...
                    <div className="absolute inset-x-3 bottom-3 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 flex gap-2 z-10">
                      <button
                        ...
                        className="flex-1 py-2.5 rounded-pill bg-zinc-950 text-white text-xs font-bold uppercase tracking-wider hover:bg-amber-400 hover:text-zinc-950 transition-colors flex items-center justify-center gap-1.5 shadow-lg"
                      >
                        <ShoppingBag size={14} /> Add to Cart
                      </button>
                      <button
                        onClick={() => setQuickViewProduct(product)}
                        className="p-2.5 rounded-full bg-white text-zinc-950 hover:bg-zinc-100 transition-colors shadow-lg"
                        title="Quick View"
                      >
                        <Eye size={16} />
                      </button>
                  ...
                  <div className="p-5">
                    ...
                    <Link href={`/product/${product.slug}`}>
                      <h3 className="font-display font-bold text-lg text-text-primary group-hover:text-amber-600 transition-colors mt-0.5">
                        {product.title}
                      </h3>
                    </Link>
                    <p className="text-xs text-text-secondary mt-1 line-clamp-2">{product.subtitle}</p>
                ...
                <div className="px-5 py-3 bg-bg-muted border-t border-border flex items-center justify-between text-[11px] text-text-secondary font-medium">
