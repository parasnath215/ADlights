...
export const BestSellersCarousel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const scroll = () => {
      if (scrollContainerRef.current && !isHovered && !isManualPaused) {
  ...
  return (
    <section
      className="relative py-8 sm:py-12 bg-white text-zinc-950 border-b border-border select-none overflow-hidden"
    >
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 group/section">
        {/* Centered Header Section */}
        <div className="text-center max-w-3xl mx-auto mb-6 sm:mb-8">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-pill bg-zinc-100 border border-border text-xs font-bold uppercase tracking-widest text-amber-500 shadow-sm mb-2.5">
            <Sparkles size={13} className="text-amber-500" /> Curated Architectural Selection
          </span>
          <h2 className="font-display font-extrabold text-3xl sm:text-5xl uppercase tracking-tight text-zinc-950 mt-1">
        ...
        <div className="relative">
          ...
          >
            <div className="flex w-max gap-6 items-stretch">
          ...
          {/* Navigation Controls Left/Right Centered */}
          <button
            onClick={scrollLeft}
            className="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 p-3 rounded-full bg-zinc-900/90 border border-zinc-700 text-white hover:bg-amber-400 hover:text-zinc-950 hover:border-amber-400 transition-all shadow-md hover:scale-105 active:scale-95 cursor-pointer backdrop-blur-sm opacity-0 group-hover/section:opacity-100 hidden sm:block"
          ...
          <button
            onClick={scrollRight}
            className="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 p-3 rounded-full bg-zinc-900/90 border border-zinc-700 text-white hover:bg-amber-400 hover:text-zinc-950 hover:border-amber-400 transition-all shadow-md hover:scale-105 active:scale-95 cursor-pointer backdrop-blur-sm opacity-0 group-hover/section:opacity-100 hidden sm:block"
